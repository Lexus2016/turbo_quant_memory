...
- `remember_note(..., kind=..., scope="project")` stores a typed project note
- direct public writes to `global` are rejected
- `promote_note(note_id)` creates the reusable global copy

- `remember_note(..., kind=..., scope="project")` зберігає типізовану project-note
...
- `key_points`
- `can_hydrate`
- `note_kind` for note hits / для note-hit'ів
- `promoted_from` when relevant / коли релевантно

Default retrieval does **not** include raw excerpts or whole-file dumps. That boundary keeps token volume low and pushes fuller context into explicit `hydrate(...)` calls.

Дефолтний retrieval **не** містить raw excerpts або дампів цілих файлів. Ця межа тримає token volume низьким і переносить повніший контекст в явні виклики `hydrate(...)`.
...
## 9.5 Hydration Strategy / Стратегія hydration

Hydration is explicit and bounded:

Hydration є явним і обмеженим:

- `hydrate(..., mode="default")` returns the target plus `before=1` / `after=1`
- `hydrate(..., mode="related")` returns the target plus `before=2` / `after=2`
- Markdown hydration stays file-local
- Note hydration returns the full note body plus `note_kind`, `tags`, and `source_refs`

- `hydrate(..., mode="default")` повертає target плюс `before=1` / `after=1`
- `hydrate(..., mode="related")` повертає target плюс `before=2` / `after=2`
- Markdown hydration лишається file-local
- Note hydration повертає повне note body разом із `note_kind`, `tags` і `source_refs`
