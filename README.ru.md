...
* **Никогда не пиши через `set_secret` значение, которое уже в чате:** если пользователь вставил credential в чат, попроси задать из терминала: `turbo-memory-mcp secret-set NAME` читает через `getpass` со скрытым вводом — значение не попадает в транскрипт или shell-историю.
