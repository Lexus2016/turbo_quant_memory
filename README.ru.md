...
### 4. Лёгкий ONNX-бэкенд (мало RAM, опционально)
По умолчанию эмбеддер работает на PyTorch. На слабой машине (напр. ~2 ГБ RAM) ту же многоязычную модель можно запустить через ONNX Runtime — это убирает тяжёлый след PyTorch ради значительно меньшего объёма памяти, **без изменения качества и без переиндексации** (векторы остаются совместимыми).

```bash
pip install 'turbo-memory-mcp[onnx]'
export TQMEMORY_EMBEDDING_BACKEND=fastembed   # по умолчанию: sentence-transformers
```

Стандартная установка не меняется — это чисто опционально.
...
3. **Получение контекста:** Для конкретной задачи выполни `semantic_search(query="<тема_задачи>", scope="hybrid")` для получения ранее сохраненных архитектурных решений, правил оформления кода, уроков или инструкций. Чтобы достать handoff запросом, передай `tier_filter=["episodic"]`.
