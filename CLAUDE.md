...
- If you encounter a `master_key_unavailable` or `master_key_mismatch` error response, surface its `setup_hint` verbatim to the user and stop; do not try to recover by guessing keys. `master_key_mismatch` (v0.11.0+) means the resolved key does not match the vault — usually a `TQMEMORY_SECRETS_PASSPHRASE` shadowing a keyring-keyed vault (the env var is an Argon2id passphrase, not the raw keyring key); the fix is normally to unset that env var. Reads never mint a new key, so the vault stays intact.
