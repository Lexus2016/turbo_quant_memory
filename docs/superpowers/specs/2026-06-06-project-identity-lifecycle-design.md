...
- **Date:** 2026-06-06
- **Status:** Feature 1 (identity stability) **implemented** 2026-06-06. Feature 2 orphan **detection** implemented 2026-06-06 (surfaced in `server_info`); assisted **prune** deferred (manual `mv`-to-staging suffices). See "What shipped".
...
**Feature 2 — orphan detection (shipped):** `store.detect_orphaned_buckets(storage_root)` lists buckets whose `project_root` is gone (`{project_id, project_name, project_root, note_count}`); read-only, never deletes. Surfaced as `orphaned_buckets` in `server_info` (the diagnostic call — NOT `health`, which is a liveness probe). `build_server_info_payload` gained an optional `orphaned_buckets` arg. Tests `tests/test_orphan_detection.py` (3: missing-root flagged + live skipped, empty when no projects dir, end-to-end via `server_info`); verified on real storage (flags exactly the 2 known orphans). Assisted **prune** (move-to-staging + confirm delete) deferred — the manual `mv` path covers the rare cleanup; automating destruction is not yet necessary. Full suite: 361 passed.
