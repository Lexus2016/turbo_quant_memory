...
Швидкі посилання: [Що це робить](#що-це-робить) | [Встановлення](#встановлення) | [Підключення клієнта](#підключення-клієнта) | [Бенчмарки](#бенчмарки-з-цього-репозиторію) | [Гайд по клієнтах](CLIENT_INTEGRATIONS.uk.md) | [Технічна специфікація](TECHNICAL_SPEC.uk.md) | [Стратегія пам’яті](MEMORY_STRATEGY.uk.md)
...
| 6. Тримати базу знань чистою | `lint_knowledge_base` знаходить биті посилання, сирітські нотатки і дублікати заголовків |

## Бенчмарки З Цього Репозиторію

У репозиторії є реальний запуск бенчмарку в [benchmarks/latest.uk.md](benchmarks/latest.uk.md) та [benchmarks/latest.json](benchmarks/latest.json).

![Знімок benchmark](benchmarks/summary-uk.svg?v=20260328b)
...
| Для чого | Команди |
|---|---|
| Встановлення релізу через `uv` | `uv tool install git+https://github.com/Lexus2016/turbo_quant_memory@v0.2.4`<br>`turbo-memory-mcp serve` |
...
Чекліст smoke-перевірки: [examples/clients/SMOKE_CHECKLIST.uk.md](examples/clients/SMOKE_CHECKLIST.uk.md)
...
| `lint_knowledge_base` | Запустити структурну перевірку markdown knowledge base |
...
| `lint_knowledge_base` | Показати биті internal links, orphan candidates і duplicate titles |
...
- Гайд по клієнтах: [CLIENT_INTEGRATIONS.uk.md](CLIENT_INTEGRATIONS.uk.md)
