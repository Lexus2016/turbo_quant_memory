# 🧠 Turbo Quant Memory для ШІ-агентів (v0.24.1)

> **Тримовна локальна пам'ять та граф знань для ШІ-агентів розробки, що встановлюється автоматично.** Агенти отримують компактні картки результатів замість перечитування цілих файлів — постійне, локальне та зв'язане знання.
...
Замість того, щоб щоразу перечитувати вихідні документи, ваш ШІ-агент використовує **Компактне вилучення (Compact Retrieval)**: кожен пошук повертає малі картки результатів (~220-символьні передпрогляди), а повний вміст підтягується лише через hydrate за потреби.
...
> "Привіт! Будь ласка, встанови та налаштуй мені сервер розумної пам'яті Turbo Quant Memory для моєї робочої області за допомогою цього репозиторію: https://github.com/Lexus2016/turbo_quant_memory. Прочитай README.uk.md, виконай 'Інструкції для ШІ-агентів' у самому низу файлу, щоб встановити пакет через `uv tool`, зареєструвати MCP-сервер `tqmemory`, виконати `turbo-memory-mcp skill install`, запустити перевірку здоров'я, проіндексувати цей проект та налаштувати нашу постійну пам'ять. Повідом мені, коли все буде готово!"
