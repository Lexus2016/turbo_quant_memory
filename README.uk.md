# 🧠 Turbo Quant Memory для ШІ-агентів (v0.20.0)
...
**Живий зріз з реального інстансу розробника (v0.20.0):**

| Що зробила пам'ять | Число |
| :--- | :--- |
| 🔢 Заощаджено вхідних токенів (сумарно) | **≈ 980 000** і зростає |
| 🔁 Оброблено запитів | **883** пошуків + **179** глибоких підвантажень |
| 📉 Середня економія на запит | **≈ 1 100 токенів** |
| 📚 Знань під керуванням | **206** активних нотаток + **440** проіндексованих блоків коду |
...
### 4. Легкий ONNX-бекенд (мало RAM, опційно)
...
export TQMEMORY_EMBEDDING_BACKEND=fastembed   # за замовчуванням: sentence-transformers
```

Стандартна інсталяція не змінюється — це суто опційно.
...
3. **Отримання контексту:** Для конкретної задачі виконай `semantic_search(query="<тема_задачі>", scope="hybrid")` для отримання раніше збережених архітектурних рішень, правил оформлення коду, уроків чи інструкцій. Щоб дістати handoff запитом, передай `tier_filter=["episodic"]`.
