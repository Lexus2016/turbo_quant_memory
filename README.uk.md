# 🧠 Turbo Quant Memory для ШІ-агентів (v0.23.0)
...
| **Автозбагачення контексту** | 🔄 **Зв'язки у відповіді** | Пов'язані файли, нотатки та завдання потрапляють у результати `semantic_search` / `hydrate` — без зайвих запитів |
...
**Живий зріз з реального інстансу розробника (v0.23.0):**

| Що зробила пам'ять | Число |
| :--- | :--- |
| 🔢 Заощаджено вхідних токенів (сумарно) | **≈ 980 000** і зростає |
| 🔁 Оброблено запитів | **885** пошуків + **179** глибоких підвантажень |
| 📉 Середня економія на запит | **≈ 1 100 токенів** |
| 📚 Знань під керуванням | **207** активних нотаток + **440** проіндексованих блоків коду |
...
2. **Продовж з місця зупинки:** На старті сесії — або одразу після стиснення контексту — спершу виклич `recent_context()`. Це bootstrap без запиту: повертає найсвіжіші нотатки (новіші перші), **включно з нотатками `handoff`**, які звичайний `semantic_search` за замовчуванням приховує. Це надійна точка входу «де я зупинився», коли ти ще не знаєш, що шукати.
3. **Отримання контексту:** Для конкретної задачі виконай `semantic_search(query="<тема_задачі>", scope="hybrid")` для отримання раніше збережених архітектурних рішень, правил оформлення коду, уроків чи інструкцій. Щоб дістати handoff запитом, передай `tier_filter=["episodic"]`. Коли питаєш "що ми вирішили/чого навчились про X" — передай `source_filter="notes"`, щоб проіндексовані markdown-блоки не витісняли нотатки-рішення з топу видачі.
...
* Після збереження нотатки про виправлення багу у файлі `src/auth.py` виклич `link_entities(source_uri="note://<note_id>", target_uri="file://src/auth.py", relation_type="fixes")`. URI файлів задаються **відносно кореня проєкту** (не абсолютні).
* Зв'язуй пов'язані нотатки: `link_entities(source_uri="note://<new_note>", target_uri="note://<old_note>", relation_type="supersedes")` (замінює).
* Зв'язуй файли із завданнями: `link_entities(source_uri="file://src/auth.py", target_uri="task://TASK-101", relation_type="implements")`.
