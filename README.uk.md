# Turbo Quant Memory for AI Agents

![Титульна ілюстрація Turbo Quant Memory](assets/readme-hero-uk.svg?v=20260328b)

[![Latest release](https://img.shields.io/github/v/release/Lexus2016/turbo_quant_memory?display_name=tag&label=release)](https://github.com/Lexus2016/turbo_quant_memory/releases)
[![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-3776AB)](https://www.python.org/downloads/)
[![MCP server](https://img.shields.io/badge/MCP-stdio-0A7B83)](https://modelcontextprotocol.io/)
[![Local-first](https://img.shields.io/badge/storage-local--first-2F855A)](https://github.com/Lexus2016/turbo_quant_memory)

Інші мови: [English](README.md) | [Russian](README.ru.md)

Turbo Quant Memory — це шар пам’яті, який перетворює AI-агента з “розумного, але короткозорого помічника” на стабільного члена команди.

Якщо ви працюєте з Claude Code, Codex, Cursor, OpenCode, Gemini CLI або будь-яким MCP-клієнтом, саме так зберігається інституційна пам’ять вашого продукту.

## Чому Це Важливо

Більшість AI-флоу ламаються в одному місці: пам’ять.

- Корисні висновки губляться в чатах.
- Кожна нова задача стартує майже з нуля.
- Команда знову і знову пояснює одне й те саме.

Turbo Quant Memory робить знання постійними, пошуковими та придатними до повторного використання.

## Чому Команди Обирають Turbo Quant Memory

| Типовий AI-процес | З Turbo Quant Memory |
|---|---|
| Агент забуває контекст між сесіями | Агент продовжує роботу з уже збережених знань |
| Рішення заховані в старих тредах | Рішення стають нотатками, які легко знайти |
| Знання тримається на окремих людях | Знання стає спільним активом команди |
| Контекст витрачається на повторне читання | Більше бюджету залишається на міркування |

## Головна Обіцянка

Ваші агенти перестають бути “тимчасовими чат-асистентами” і починають працювати як повноцінні учасники команди.

## Що Робить Його Особливим

- Local-first підхід: пам’ять під вашим контролем.
- Один шар пам’яті для багатьох клієнтів.
- Безперервність між агентами: можна почати в Codex, продовжити в Gemini CLI і повернутися в Codex з тією самою project memory.
- Орієнтація на реальну розробку: рішення, патерни, handoff-и.
- Прозорість: знання структуроване й перевіряється.

## Швидкий Старт

Використайте цей сценарій на 60 секунд:

1. Встановіть один раз:
```bash
uv tool install git+https://github.com/Lexus2016/turbo_quant_memory@v0.6.0
```

2. Додайте MCP-сервер `tqmemory` у клієнт (клієнт запускатиме його автоматично):

```bash
# Codex
codex mcp add tqmemory -- turbo-memory-mcp serve

# Gemini CLI
gemini mcp add tqmemory turbo-memory-mcp serve

# Claude Code (project scope)
claude mcp add --scope project tqmemory -- turbo-memory-mcp serve
```

3. Перезапустіть клієнт і викличте будь-який інструмент `tqmemory`.

Потрібен готовий конфіг для Gemini CLI, Cursor, OpenCode або Antigravity? Візьміть його з [CLIENT_INTEGRATIONS.uk.md](CLIENT_INTEGRATIONS.uk.md).

### Оновлення

Щоб підтягнути новий реліз у вже встановлене дерево, повторіть команду встановлення з `--reinstall`:

```bash
uv tool install --reinstall git+https://github.com/Lexus2016/turbo_quant_memory@v0.6.0
```

Після оновлення з v0.4.x запустіть `turbo-memory-mcp migrate --status` — побачите pending Phase 2 (`notes` v1→v2 + `retrieval` v1→v2). Зупиніть усі MCP-клієнти (lockfile демона блокує `--apply`, поки primary працює) і виконайте `turbo-memory-mcp migrate --apply`. Перед застосуванням робиться rolling snapshot; у разі помилки CLI друкує точну команду `--restore-from`. Повний список — у секції `Що Додано У v0.5.0` нижче.

Якщо `~/.gemini/settings.json` був створений до v0.4.2, разово злийте до нього цей блок, щоб Gemini CLI почав читати `AGENTS.md` поруч із `GEMINI.md`:

```jsonc
{
  "context": { "fileName": ["AGENTS.md", "GEMINI.md"] }
}
```

## Ігнорування Файлів При Індексації

Створіть файл `.tqmemoryignore` в корені проєкту, щоб виключити директорії або файли з Markdown-індексації. Формат аналогічний `.gitignore` — один glob-патерн на рядок, `#` для коментарів.

```gitignore
# Пропустити дублі workspace-шаблонів
workspace-*

# Пропустити runtime-звіти
data/reports/*.md

# Пропустити згенерований контент
output/
```

Ignore-файл підхоплюється автоматично при виклику `index_paths(...)`. Патерни зіставляються як з іменами директорій, так і з повними відносними шляхами. Пошук іде вгору від індексованого кореня до `.tqmemoryignore` або межі `.git`.

Директорії `node_modules`, `.git`, `__pycache__`, `dist` та `build` завжди ігноруються за замовчуванням.

## Спільна Памʼять Між Агентами

У стандартному локальному встановленні це працює з коробки. Не потрібні окремий sync-сервіс, export/import або окрема memory-конфігурація під кожного агента.

Це спільна локальна памʼять, а не віддалена хмарна синхронізація. Якщо Codex і Gemini CLI працюють на одній машині та відкривають той самий репозиторій, вони автоматично використовують один і той самий memory layer.

Щоб мати одну спільну project memory між Codex, Gemini CLI та іншими MCP-клієнтами:

1. Один раз встановіть `turbo-memory-mcp` на машині.
2. Додайте той самий MCP-сервер `tqmemory` у кожен клієнт, яким користуєтесь.
3. Відкривайте той самий репозиторій у кожному клієнті.

Коли ці умови виконані, клієнти автоматично бачать одну й ту саму project memory. Тобто можна почати роботу в Codex, продовжити в Gemini CLI, а потім повернутися в Codex без повторного відновлення контексту.

Якщо якийсь клієнт стартує не з кореня репозиторію, явно задайте `TQMEMORY_PROJECT_ROOT`, щоб він визначав той самий project identity.

## Для Кого Це

- інженерні команди, що будують AI-first процеси
- соло-розробники з кількома агентами
- продуктові команди, яким потрібна стабільна якість AI-виконання
- усі, хто втомився повторювати контекст щодня

## Чому Треба Обрати Саме Це

Turbo Quant Memory дає:

- швидший старт кожної нової задачі
- менше повторних помилок
- безперервність між сесіями
- вищу віддачу від кожного запуску агента

## Перевага, Підтверджена Бенчмарками Та Економією Витрат

![Зведення бенчмарків](benchmarks/summary-uk.svg)

На реальному корпусі цього репозиторію компактний memory-шлях показує сильну економію, яка напряму зменшує витрати на модель:

- лише `semantic_search`: у середньому **-63.96% байтів** до моделі
- `semantic_search + hydrate(top1)`: у середньому **-44.1% байтів**
- середня затримка `semantic_search`: **68.13 мс**
- середня затримка `hydrate`: **41.63 мс**

Чому це практична перевага:

- менше повторного читання означає менше платних вхідних токенів
- нижчий токен-обсяг означає нижчу вартість кожної задачі
- контекстний бюджет іде на міркування, а не на повторне завантаження файлів

## Що Додано У v0.5.0

Цей реліз приносить перші дві фази milestone Memory Quality v1: фреймворк міграції схеми (Phase A) і розділення нот на tier-и (Phase 2). Існуючі v0.4.x встановлення оновлюються на місці — зворотно сумісно і з можливістю відкату з rolling snapshot.

- **Розділення на tier-и.** Кожна нотатка тепер має `tier`: `durable` (рішення / патерни / уроки), `episodic` (session handoff) або `reference` (markdown-блоки). Дефолтний `semantic_search` повертає тільки `durable + reference`, тож handoff-и більше не топлять довговічні знання. Явно ввімкнути episodic — `tier_filter=("episodic",)`.
- **Tier у MCP видачі.** Відповіді `semantic_search` і `hydrate` тепер містять поле `tier` на кожному елементі — клієнти можуть гілкуватися за одним значенням.
- **Фреймворк міграції схеми.** Нова CLI-команда `turbo-memory-mcp migrate` з опціями `--status`, `--dry-run`, `--apply`, `--snapshot-only`, `--list-snapshots`, `--restore-from <path>`, `--force` (обхід захисту lockfile). Rolling snapshot під `<storage_root>/.snapshots/` з настроюваною ретенцією (`TQMEMORY_SNAPSHOTS_KEEP`). Структурований JSONL-лог у `~/.turbo-quant-memory/migration.log`.
- **Детекція на старті демона.** Primary daemon детектить pending апгрейди при запуску і друкує однорядкове попередження у stderr. Детекція ніколи не блокує старт. Proxies пропускають перевірку.
- **Graceful upgrade.** До запуску `migrate --apply` нова WHERE-фраза за tier пропускається на старих LanceDB-таблицях, тож пошук продовжує працювати між апгрейдом бінарника і `migrate`.
- **Sticky format_version у manifest.** Manifest-write тепер зберігає bump'нуту версію (`max(existing, in-code baseline)`), тож пост-міграційні writes не можуть тихо відкотити версію і re-trigger apply loop. Stale `format_version=0` так само auto-repair'иться.
- **`promote_note` зберігає tier.** Явно встановлений tier на project-ноті виживає при промоції у global.
- **`.claude/` ігнорується за замовчуванням.**
- **CHANGELOG.md** ведеться у форматі Keep a Changelog.

## Що Додано У v0.4.3

- Технічна базова крапка перед наступним архітектурним циклом — без зміни поведінки.
- Додано `CHANGELOG.md` із повною історією релізів у форматі Keep a Changelog.
- Каталог `.claude/` додано до `.gitignore`, щоб локальна конфігурація Claude Code не потрапляла у репозиторій.
- Install-команди в README і SMOKE checklist оновлено на `v0.4.3`.

## Що Додано У v0.4.2

- Фікстура Gemini CLI і вбудований `.gemini/settings.json` тепер постачаються з `"context": {"fileName": ["AGENTS.md", "GEMINI.md"]}`, тож Gemini CLI підхоплює ті самі project-промпти з `AGENTS.md`, що і решта агентів — без дублюючого `GEMINI.md`.
- Install-команди в README та SMOKE checklist тепер вказують на фактичний поточний реліз, плюс додано підрозділ `Оновлення` з `uv tool install --reinstall` і разовою міграцією `~/.gemini/settings.json`.
- Новий крок у SMOKE checklist попереджає, що при merge фікстури Gemini в наявний `settings.json` блок `context` обовʼязково треба зберегти — інакше Gemini CLI тихо повертається до `GEMINI.md`-only й пропускає `AGENTS.md`.
- Зафіксовано `.planning/todos/2026-04-28-lint-false-positives.md` з двома знайденими багами `lint_knowledge_base` під наступний кодовий реліз: ASCII-нормалізація title-key, що складає Cyrillic / не-ASCII H1 у `untitled`, і `broken_link` для файлів усередині `DEFAULT_IGNORED_DIR_NAMES` (наприклад `benchmarks/latest.md`), які реально існують на диску.

## Що Додано У v0.4.1

- Автоматичний failover проксі-процесів, коли primary помирає. Раніше, якщо перший `turbo-memory-mcp` процес (primary, що тримає sentence-transformers модель і LanceDB-дескриптори) вимикався, решта проксі в інших MCP-клієнтах втрачали RPC-зв'язок і `tqmemory` ставав тихо недоступним у цих клієнтах.
- Проксі тепер детектують зниклий primary через `PrimaryUnreachable` при наступному RPC-виклику і прозоро ре-бутстрапляться: один із вцілілих проксі атомарно забирає lockfile та промотує себе в primary (стартує власний `DaemonListener`), а кожен інший осиротілий проксі переконнектується до нового primary. Перезапуск MCP-клієнтів не потрібен.
- Phase-aware обробка RPC-помилок: помилки фази connect/send транслюються в `PrimaryUnreachable` (безпечно для повтору), а помилки mid-call (send пройшов, recv впав) піднімаються до хоста без змін, щоб non-idempotent інструменти як `remember_note` ніколи не дублювалися.
- Фікстура Gemini CLI тепер постачається з `context.fileName: ["AGENTS.md", "GEMINI.md"]`, тож Gemini CLI підхоплює ті самі project-промпти з `AGENTS.md`, що й Codex, Cursor і решта агентів, без потреби в дублюючому `GEMINI.md`.

## Що Додано У v0.4.0

- Транспорт через singleton-daemon: лише один процес `turbo-memory-mcp` на машині тримає sentence-transformers модель і LanceDB-дескриптори. Кожен наступний запуск MCP-клієнта стає тонким stdio↔socket проксі, що пересилає виклики інструментів до головного.
- Крос-платформна координація: на Unix/macOS використовується `AF_UNIX` сокет у системній tmp-директорії (короткий шлях, права 0600); на Windows — named pipe поточного користувача. Аутентифікація — 32-байтовий випадковий authkey у `~/.turbo-quant-memory/.daemon.lock`.
- Lazy-імпорти у `retrieval_index` означають, що проксі не платить ~470 MB за імпорти PyTorch / LanceDB / PyArrow, якщо лише пересилає RPC. Виміряна економія: ~1 ГБ RSS для чотирьох одночасних MCP-клієнтів (primary 530 MB, проксі ~50 MB кожен проти 437 MB кожен раніше).
- Існуючий стан на диску (JSON-нотатки, LanceDB-таблиці, Markdown-блоки, manifests) без змін і повністю зворотно-сумісний. Escape hatch: встановити `TQMEMORY_DAEMON_DISABLE=1`, щоб повернутись у per-process режим.

## Що Додано У v0.3.1

- Задокументовано shared-memory handoff між Codex і Gemini CLI в README та client integration docs.
- Додано готову Gemini CLI фікстуру і smoke-check кроки для перевірки того самого `tqmemory` сервера в різних клієнтах.
- Чітко пояснено, що shared memory означає локальну безперервність роботи на одній машині, а не віддалену хмарну синхронізацію.

## Де Подивитися Деталі

- Інтеграції з клієнтами: [CLIENT_INTEGRATIONS.uk.md](CLIENT_INTEGRATIONS.uk.md)
- Технічна специфікація: [TECHNICAL_SPEC.uk.md](TECHNICAL_SPEC.uk.md)
- Стратегія пам’яті: [MEMORY_STRATEGY.uk.md](MEMORY_STRATEGY.uk.md)
- Бенчмарки: [benchmarks/latest.uk.md](benchmarks/latest.uk.md)
