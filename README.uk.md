...
Потрібні Cursor, OpenCode або Antigravity? Візьміть готові конфіги з [CLIENT_INTEGRATIONS.uk.md](CLIENT_INTEGRATIONS.uk.md).
...
- Версіоновані index manifest-и автоматично перебудовують похідні індекси після format-changing апдейту.
- `server_info()` тепер показує постійну usage/savings telemetry окремо від memory scope.
- Задайте `TQMEMORY_INPUT_COST_PER_1M_TOKENS_USD`, якщо хочете бачити приблизну економію в USD.
- Retrieval-відповіді можуть час від часу показувати короткі savings milestone без засмічення памʼяті.
