...
Агенти НЕ ПОВИННІ викликати `set_secret` зі значенням, яке потрапило у чат-контекст (де його уже зафіксував MCP-клієнт). Рекомендований патерн:
1. Спочатку шукати у project memory `pattern`-нотатку-рецепт із поясненням, як дістати credential (наприклад: `"Щоб приконнектитися до prod DB, виклич get_secret('prod-db-dsn')"`).
2. Викликати `get_secret(name)` тільки коли значення дійсно потрібне; передавати `secret_value` програмно, ніколи не друкувати у summary чи логах.
3. Просити користувача задати секрет уперше з терміналу — `turbo-memory-mcp secret-set NAME` читає значення через `getpass` зі прихованим вводом — щоб значення взагалі не потрапило у чат. Прямий `keyring set` — еквівалентна low-level альтернатива.
