...
4. додає невеликий бонус за свіжість (≤ 0.05, спадає за 30 днів), щоб нещодавно оновлені записи вигравали близькі перегони у старих записів зі схожою лексикою
5. фінально розв'язує нічию за пріоритетом `project`, потім за новішим `updated_at`, потім за стабільною ідентичністю
...
## Секрети vs. Нотатки (Phase 9)

Секретний vault — це ОКРЕМА підсистема з іншою семантикою, відмінною від нотаток. Тримайте їх ментально та операційно роздільно:

| | Нотатки | Секрети |
|---|---|---|
| Зберігання | plaintext JSON, індексується | AES-256-GCM blob, ніколи не індексується |
| Шлях retrieval | `semantic_search` / `hydrate` (нечіткий пошук) | тільки `get_secret(name)` (точне ім'я) |
| Scope | project + global | тільки project (без параметра `scope`) |
| Promotion | `promote_note` у global | не підтримується (завжди per-project) |
| Передача | ніколи не залишає машину (нуль outbound HTTP) | ніколи не залишає машину (та сама гарантія) |

Поведінка агента для vault'а:
1. **Спочатку рецепти.** Шукай правильний виклик `get_secret(name)` через `semantic_search` за `pattern`-нотаткою-рецептом, що документує credential. Ніколи не виловлюй імена з історії чату.
2. **Читай через `secret_value`.** Викликай `get_secret(name)` лише коли значення дійсно потрібне; передавай `secret_value` програмно (env injection, аргумент підпроцесу); не дублюй у summary, логи, `remember_note`.
3. **Пиши реактивно для чат-видимих значень.** Якщо користувач щойно вставив credential, або ти сам згенерував його у розмові — викликай `set_secret(name, value)` напряму. У тебе авторитетний активний `project_id` через cwd resolution; користувач, що запускає CLI з терміналу, не завжди. Відсилати користувача знову у CLI переробляти значення, яке вже у транскрипті — це фрикція без захисту.
4. **CLI рекомендуй профілактично.** Поки значення ще НЕ у чаті — наприклад, користувач питає "куди мені покласти SSH-ключ?" — направ його на `turbo-memory-mcp secret-set NAME` з терміналу (`getpass` зі прихованим вводом). Прямий `keyring set` — low-level еквівалент. Після того як значення вже у чаті, CLI не дає додаткової секретності.
...
- Ніколи не записуйте значення секретів у нотатки (нотатки індексуються; секрети мають іти у vault).
- Ніколи не викликайте `get_secret` з ім'ям, виловленим з чат-транскрипту — спочатку прочитайте його з `pattern`-нотатки-рецепту.
