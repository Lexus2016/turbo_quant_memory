...
3. Просити користувача задати секрет уперше через `turbo-memory-mcp` CLI (запланована команда) або через `keyring set` напряму, щоб значення взагалі не потрапило у чат.
